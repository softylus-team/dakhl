...
class TransactionBuilder
{
    ...
    public function create(array $transactionData)
    {
        ...
        $transaction = $this->owner->HyperpayTransactions()->create([
    ...
    {
        $transaction = $this->owner->HyperpayTransactions()->where('status', 'pending')->whereBrand($this->getBrand($transactionData['entityId']))->first();
        if ($transaction) {
            $transaction->delete();
        }
    }
